...
$search = "SELECT s.item_id, s.item_name, s.item_description, s.item_price, s.likes, s.dislikes, s.teacher, s.bought_times, s.item_type, s.item_subject, s.school_class, s.item_answers FROM shop s, users u WHERE u.school_id = s.school_id AND u.user_id = $user_id AND s.visible = 1 AND s.checked = 1";
$total_pages_sql = "SELECT COUNT(*) FROM shop s, users u WHERE u.school_id = s.school_id AND u.user_id = $user_id AND s.visible = 1 AND s.checked = 1";
