...
$search = "SELECT u.user_id, u.username, u.register_date, u.bought_items, r.referrals_money FROM users u, referrals r WHERE r.referrals_userby = $user_id AND r.referrals_userid = u.user_id AND u.bought_items > 0";
$total_pages_sql = "SELECT COUNT(*) FROM users u, referrals r WHERE r.referrals_userby = $user_id AND r.referrals_userid = u.user_id AND u.bought_items > 0";
